...
async def ai_check(text, mode="is_seeker"):
    ...
    try:
        if mode == "is_seeker":
            sys_prompt = "Ты HR ищешь лиды и клиентов. Ответь ДА, только если человек САМ ищет работу, переезд, переход, дополнительный доход. НЕТ — если это вакансия, реклама услуг или спам."
        else:
