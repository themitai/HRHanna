...
async def ai_check(text, mode="is_seeker"):
    ...
    try:
        if mode == "is_seeker":
            sys_prompt = (
                "Ты — фильтр входящего трафика. Твоя задача: отличить Покупателя от Продавца. "
                "Ответь 'ДА', только если автор сообщения — реальный человек, который: "
                "1. Задает вопросы о том, как начать (арбитраж, трейдинг, крипта). "
                "2. Просит совета, ищет наставника или обучение. "
                "3. Пишет о потере денег и поиске надежного способа заработка. "
                "4. Ищет 'вход' в закрытые каналы или чаты с сигналами. "
                "Ответь 'НЕТ', если сообщение содержит: "
                "- Предложение услуг, софта, обучения или каналов. "
                "- Ссылки на другие группы/ботов. "
                "- Фразы типа 'научу', 'покажу', 'пишите в ЛС', 'есть профит', 'набираю команду'. "
                "- Вакансии (ищу воркеров, менеджеров)."
            )
        else:
            sys_prompt = (
                "Проанализируй ответ клиента. Он проявляет готовность продолжить общение или задает уточняющие вопросы? "
                "ДА — если спрашивает про цену, формат, гарантии или говорит 'расскажи подробнее'. "
                "НЕТ — если пишет 'дорого', 'не интересно', 'подумаю' (без вопросов) или просто присылает смайл."
             )
    ...
    if event.is_private:
        ...
        if status in ("sent", "offered"):
            ...
            if await ai_check(text, "is_interest"):
                ...
                try:
                    if status == "sent":
                        await event.reply("Опишите подробнее свой запрос!")
